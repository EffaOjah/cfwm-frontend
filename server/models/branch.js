    ...
    create: async (data) => {
        ...
        await pool.query(
            `INSERT INTO branches (id, district_id, name, address, map_url, pastor, phone, image_url, is_headquarters) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, district_id || null, name, address, map_url || null, pastor, phone, image_url, is_headquarters || false]
        );
        return id;
    },
    ...
    update: async (id, data) => {
        ...
        await pool.query(
            `UPDATE branches SET district_id = ?, name = ?, address = ?, map_url = ?, pastor = ?, phone = ?, image_url = ?, is_headquarters = ? 
             WHERE id = ?`,
            [district_id || null, name, address, map_url || null, pastor, phone, image_url, is_headquarters || false, id]
        );
    },
